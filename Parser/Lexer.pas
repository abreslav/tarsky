...
type
  ...
  TGrammarElementType = (gQuantor, gIneqSign, gPlusOp, gMelOp, gOper, gPower, gVar, gNumber, gNumber16, gEnd, gError, gBracketOpen, gBracketClose, gBracketFigureOpen, gBracketFigureClose, gBracketSquareOpen, gBracketSquareClose, gExc);
  TCharLexer =(cVar, cNum, cNum16, cPlusOp, cMelOp, cPower, cSing, cGap, cError, cEnd, cBracketOpen, cBracketClose, cBracketFigureOpen, cBracketFigureClose, cBracketSquareOpen, cBracketSquareClose, cExc);
...
function number16( c: char) : boolean;
begin
  case c of
    'A'..'F', '0'..'9': result := true;
  else
...
begin
  ...
  case GrammarState of
    ...
    gNumber16: if number16(lastChar) then
        operStr
      else begin
        GrammarState := gNumber;
        lexerState := lsEndOp;
